...
# We use our own copy to fix numpy version issues
from medpy_binary import dc, hd, asd
...
def inference(net, img_batch, modelName, epoch):
    ...
    for i, data in enumerate(img_batch):
        ...
        tp_b, fp_b, fn_b, tn_b = smp.metrics.get_stats(
            masks, segmentation_classes, mode='multiclass', num_classes=4)
        # output is shape (batch_size, num_classes). Save metrics for each class independently
        for i in range(4):
            tp[i] += tp_b[:, i].sum()
            fp[i] += fp_b[:, i].sum()
            fn[i] += fn_b[:, i].sum()
    ...
    return losses.mean(), tp, fp, fn, tn
...
def inferenceTest(net, img_batch, modelName):
    ...
    for i, data in enumerate(img_batch):
        ...
        torchvision.utils.save_image(torch.cat([images.data, labels.data, masks.view(
            labels.shape[0], 1, 256, 256).data/3.0]), os.path.join(path, str(i)+'.png'), padding=0)
